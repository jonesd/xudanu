...
impl AssertionPayload {
    pub fn type_name(&self) -> &'static str {
        match self {
            ...
            Self::SetSpanText { .. } => "SetSpanText",
            Self::DeleteSpan { .. } => "DeleteSpan",
            Self::AttachSpanToNode { .. } => "AttachSpanToNode",
            Self::DetachSpanFromNode { .. } => "DetachSpanFromNode",
            Self::CreateAnnotation { .. } => "CreateAnnotation",
            Self::AttachAnnotationToNode { .. } => "AttachAnnotationToNode",
            Self::AttachAnnotationToSpan { .. } => "AttachAnnotationToSpan",
    ...
    pub fn summary(&self) -> String {
        match self {
            Self::CreateNode { node_id, kind } => {
                format!("node={} kind=\"{}\"", node_id.as_u64(), kind)
            }
            Self::SetSpanText { span_id, text } => {
                format!("span={} text=\"{}\"", span_id.as_u64(), text)
            }
            Self::AttachChild {
                parent_id,
                child_id,
            ...
            Self::CreateSpan { span_id } => format!("span={}", span_id.as_u64()),
            Self::AttachSpanToNode {
                node_id,
                span_id,
                ordinal,
            } => {
            ...
            Self::DeleteNode { node_id } => format!("node={}", node_id.as_u64()),
            Self::DeleteSpan { span_id } => format!("span={}", span_id.as_u64()),
            Self::DetachChild {
                parent_id,
                child_id,
            } => {
            ...
            Self::DetachSpanFromNode {
                node_id,
                span_id,
            } => {
                format!("node={} span={}", node_id.as_u64(), span_id.as_u64())
            }
            ...
            Self::AttachAnnotationToNode {
                annotation_id,
                node_id,
            } => {
                format!("ann={} node={}", annotation_id.as_u64(), node_id.as_u64())
            }
            Self::AttachAnnotationToSpan {
                annotation_id,
                span_id,
            } => {
                format!("ann={} span={}", annotation_id.as_u64(), span_id.as_u64())
            }
            Self::DeleteAnnotation { annotation_id } => {
                format!("ann={}", annotation_id.as_u64())
            }
        }
    }
}
