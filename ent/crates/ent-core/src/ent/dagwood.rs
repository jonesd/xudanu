...
pub struct BranchDebugInfo {
    ...
    pub parents: Option<Vec<TracePosition>>,
}
...
impl DagWood {
    ...
    pub fn debug_branches(&self) -> Vec<BranchDebugInfo> {
        let mut result = Vec::new();
        for (id, branch) in self.branches.iter_materialized() {
            let parents = match &branch.kind {
                BranchKind::Root => None,
                BranchKind::Tree { parent } => Some(vec![*parent]),
                BranchKind::Dag { parent1, parent2 } => Some(vec![*parent1, *parent2]),
            };
            result.push(BranchDebugInfo {
                branch_id: id.as_u64(),
                last_position: branch.last_position,
                parents,
            });
        }
        result.sort_by_key(|b| b.branch_id);
        result
    }
    ...
    pub fn extend_branch(&mut self, branch_id: BranchId) -> TracePosition {
