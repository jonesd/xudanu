...
fn parse_attestation(s: &mut Stream, out: &mut ReceiptInfo) -> Result<(), String> {
    ...
    if tag == PENDING_TAG {
        // Calendars prefix the URI with a length byte that survives
        // some streams — trim a stray leading length glyph.
        let mut uri = String::from_utf8_lossy(payload).into_owned();
        if uri.starts_with('-') || uri.starts_with(',') {
            uri.remove(0);
        }
        out.pending_calendars.push(uri);
...
mod tests {
    ...
    pub(crate) const LIVE_PENDING: &[u8] = &[
        ,
    ];
...
/// Test support: a valid pending stream (live receipt fixture).
#[cfg(test)]
pub mod tests_support {
    pub fn pending_stream() -> Vec<u8> {
        super::tests::LIVE_PENDING.to_vec()
    }
}
