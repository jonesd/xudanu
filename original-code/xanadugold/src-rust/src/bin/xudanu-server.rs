    ...
    match args[1].as_str() {
        ...
        "run" => {
            ...
            if let Some(registry_path) = trusted_registry_path {
                ...
                match xudanu::crypto::server_identity::ServerRegistryFile::load_from_file(
                    ...
                    Err(e) => {
                        ...
                        if args.len() > 2 && args[1] != "init" {
                            ...
                            eprintln!(
                                "Error: Failed to load trusted registry from {}: {}",
                                registry_path, e
                            );
                            std::process::exit(1);
                        }
...

            {
                ...
                tokio::spawn(async move {
                    ...
                    loop {
                        ...
                        if tickets_dirty {
                            if let Err(err) = autosave_state.server.save_ticket_nonces() {
                                tracing::warn!("ticket nonce save failed: {}", err);
                            }
                        }
                        if saved > 0 {
