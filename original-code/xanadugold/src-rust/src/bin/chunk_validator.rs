...
fn main() {
    ...
    println!("Mode: {}{}", if fix { "fix " } else { "" }, if deep { "deep" } else { "integrity" });
    ...
    if fix { cleanup_stale_tmps(&chunks_dir, &mut report); }
    else { check_stale_tmps(&chunks_dir, &mut report); }
    ...
    if let Ok(root_hash) = read_current_root_hash(&data_dir) {
        ...
        if !all_set.contains(&root_hash) {
            report.error(format!("Root chunk {} not found in store", bytes_to_hex(&root_hash)));
        } else {
            if deep {
...
impl Report {
    fn new() -> Self {
        Self {
            errors: Vec::new(), warnings: Vec::new(), info: Vec::new(), fixes: Vec::new(),
    ...
    fn ok(&mut self, msg: impl Into<String>) { self.info.push(msg.into()); }
    fn fix(&mut self, msg: impl Into<String>) { self.fixes.push(msg.into()); }
    fn count(&mut self, key: &'static str) { *self.counts.entry(key).or_insert(0) += 1; }
    fn has_errors(&self) -> bool { !self.errors.is_empty() }

    fn print(&self) {
        for f in &self.fixes { println!("  FIXED: {}", f); }
        for i in &self.info { println!("  OK:    {}", i); }
        for w in &self.warnings { println!("  WARN:  {}", w); }
        for e in &self.errors { println!("  ERROR: {}", e); }
        println!();
        println!("Results:");
        for (k, v) in &self.counts { println!("  {}: {}", k, v); }
        let total: u64 = self.counts.values().sum();
        if total > 0 { println!("  total chunks: {}", total); }
        if !self.fixes.is_empty() { println!("  fixes applied: {}", self.fixes.len()); }
        if !self.warnings.is_empty() { println!("  warnings: {}", self.warnings.len()); }
        if !self.errors.is_empty() { println!("  errors: {}", self.errors.len()); }
...
fn scan_chunk_files(chunks_dir: &std::path::Path, report: &mut Report, fix: bool) {
    for path in walk_chunk_files_recursive(chunks_dir) {
        ...
        let hash_hex = match extract_hash_from_filename(name_str) {
            Some(h) => h,
            None => { report.warn(format!("Unrecognized file: {}", path.display())); continue; }
        };

        let expected_hash = match hex_to_bytes(&hash_hex) {
            Some(h) => h,
            None => { report.error(format!("Invalid hex in filename: {}", name_str)); continue; }
        };
        ...
        match std::fs::read(&path) {
            Ok(data) => {
                ...
                if actual != expected_hash {
                    report.error(format!(
                        ...
                        name_str, hash_hex, bytes_to_hex(&actual)
                    ));
                } else if data.is_empty() {
                    report.warn(format!("Empty chunk: {}", name_str));
                } else {
                    match data[0] {
                        0x50 => { report.count("edition"); report.count("total"); }
                        0x52 => { report.count("root"); report.count("total"); }
                        0x4A => {
            ...
            Err(e) => { report.error(format!("Cannot read {}: {}", name_str, e)); }
        }
        ...
        Ok(content) => match serde_json::from_str::<serde_json::Value>(&content) {
            Ok(val) => {
                if let Some(obj) = val.as_object() {
                    let hash = obj.get("current_root_hash").and_then(|v| v.as_str()).unwrap_or("");
                    ...
                    match obj.get("format_version").and_then(|v| v.as_u64()) {
                        Some(1) => {}
                        Some(v) => report.error(format!("root_manifest.json: format_version={} (expected 1)", v)),
...
fn read_current_root_hash(data_dir: &std::path::Path) -> Result<[u8; 32], String> {
    let content = std::fs::read_to_string(data_dir.join("root_manifest.json")).map_err(|e| e.to_string())?;
    let val: serde_json::Value = serde_json::from_str(&content).map_err(|e| e.to_string())?;
    let hash_hex = val["current_root_hash"].as_str().ok_or("missing current_root_hash")?;
    let bytes = hex::decode(hash_hex).map_err(|e| e.to_string())?;
    if bytes.len() != 32 { return Err(format!("root hash is {} bytes, expected 32", bytes.len())); }
    let mut arr = [0u8; 32];
...
fn validate_root_tree_integrity(
    ...
    report.ok(format!("Root chunk {} exists", &bytes_to_hex(root_hash)[..16]));

    let data = match store.read_chunk(root_hash) {
        Ok(d) => d,
        Err(e) => { report.error(format!("Cannot read root chunk: {}", e)); return; }
    };
    if data.is_empty() || data[0] != 0x52 {
        report.error(format!("Root chunk: bad tag (expected 0x52, got 0x{:02x})", data[0]));
        return;
    }
    report.ok("Root chunk: valid tag byte (0x52)");

    match xudanu::persist::root_chunk::deserialize_from_bytes::<xudanu::persist::root_chunk::ServerRootChunk>(&data) {
        Ok(root) => {
            if root.format_version != 1 {
                report.error(format!("Root chunk: format_version={} (expected 1)", root.format_version));
            } else {
                report.ok(format!("Root chunk: format_version=1, sequence={}, {} section refs",
                    root.sequence,
...
fn validate_root_tree_typed(
    ...
    if data.is_empty() || data[0] != 0x52 {
        report.error("Root chunk: bad tag"); return;
    }

    let root = match xudanu::persist::root_chunk::deserialize_from_bytes::<xudanu::persist::root_chunk::ServerRootChunk>(&data) {
        Ok(r) => r,
        Err(e) => { report.error(format!("Root chunk: deserialization failed: {}", e)); return; }
    };

    report.ok(format!("Root chunk: format_version={}, sequence={}", root.format_version, root.sequence));
    ...
    for (label, hash) in &walk_queue {
        if !all_hashes.contains(hash) {
            report.error(format!("{} chunk {} not found in store", label, bytes_to_hex(hash)));
            continue;
        }
        match try_deserialize_chunk(store, hash) {
            Ok((tag, name)) => report.ok(format!("{}: {} ({})", &bytes_to_hex(hash)[..16], label, name)),
            Err(e) => report.error(format!("{} ({}): deserialization failed: {}", &bytes_to_hex(hash)[..16], label, e)),
        }
    }
                ...
                Ok(idx) => {
                    ...
                    for entry in &idx.entries {
                        ...
                        if !all_hashes.contains(&entry.work_state_hash) {
                            report.error(format!("WorkStateChunk {} not found", bytes_to_hex(&entry.work_state_hash)));
                        } else {
                            match xudanu::persist::root_chunk::read_work_state_chunk(&entry.work_state_hash, store) {
                                Ok(ws) => report.ok(format!("  WorkStateChunk {}: kind={:?}, revisions={}",
                                    &bytes_to_hex(&entry.work_state_hash)[..16],
                                    ws.kind, ws.revision_count)),
                                Err(e) => report.error(format!("  WorkStateChunk {}: {}", &bytes_to_hex(&entry.work_state_hash)[..16], e)),
                            }
                        }
                    }
                }
                ...
                Ok(idx) => {
                    ...
                    for entry in &idx.entries {
                        ...
                        if !all_hashes.contains(&entry.club_state_hash) {
                            report.error(format!("ClubStateChunk {} not found", bytes_to_hex(&entry.club_state_hash)));
                        } else {
                            match xudanu::persist::root_chunk::read_club_state_chunk(&entry.club_state_hash, store) {
                                Ok(cs) => report.ok(format!("  ClubStateChunk {}: {}", &bytes_to_hex(&entry.club_state_hash)[..16], cs.be_id)),
                                Err(e) => report.error(format!("  ClubStateChunk {}: {}", &bytes_to_hex(&entry.club_state_hash)[..16], e)),
                            }
                        }
                    }
                }
    ...
    if let Some(h) = root.standalone_editions_hash {
        if all_hashes.contains(&h) {
            match xudanu::persist::root_chunk::read_standalone_editions_chunk(&h, store) {
                Ok(se) => report.ok(format!("StandaloneEditionsChunk: {} entries", se.entries.len())),
    ...
    if let Some(h) = root.admin_hash {
        if all_hashes.contains(&h) {
            match xudanu::persist::root_chunk::read_admin_chunk(&h, store) {
                Ok(ac) => report.ok(format!("AdminChunk: accepting={}, shutdown={}", ac.accepting_connections, ac.shutdown_requested)),
...
fn try_deserialize_chunk(
    ...
    if data.is_empty() { return Err("empty chunk".into()); }
    let tag = data[0];
    let name = match tag {
        0x50 => "edition",
        0x52 => "root_tree",
...
fn find_orphans_naive(
    ...
    for h in all_hashes { if !referenced.contains(h) { orphans.push(bytes_to_hex(h)); } }
    orphans.sort();
    for o in &orphans { report.warn(format!("Orphan: {}", o)); }
    if !orphans.is_empty() { report.warn(format!("{} orphan chunks", orphans.len())); }
}
...
fn find_orphans_typed_inner(
    ...
    if let Some(h) = root.works_index_hash {
        if let Ok(idx) = xudanu::persist::root_chunk::read_works_index_chunk(&h, store) {
            for e in &idx.entries { referenced.insert(e.work_state_hash); }
        }
    }
    if let Some(h) = root.clubs_index_hash {
        if let Ok(idx) = xudanu::persist::root_chunk::read_club_index_chunk(&h, store) {
            for e in &idx.entries { referenced.insert(e.club_state_hash); }
        }
    }
    if let Some(h) = root.standalone_editions_hash {
        referenced.insert(h);
    }
    if let Some(h) = root.admin_hash { referenced.insert(h); }
    if let Some(h) = root.system_clubs_hash { referenced.insert(h); }
...
fn collect_refs_from_root(root: &xudanu::persist::root_chunk::ServerRootChunk, refs: &mut HashSet<[u8; 32]>) {
    let mut add = |h: &Option<[u8; 32]>| { if let Some(hash) = h { refs.insert(*hash); } };
...
fn check_stale_tmps(chunks_dir: &std::path::Path, report: &mut Report) {
    ...
    if count > 0 { report.warn(format!("{} stale .tmp files (use --fix)", count)); }
}
        ...
        if let Some(n) = path.file_name().and_then(|f| f.to_str()) {
            if n.ends_with(".tmp") {
                match std::fs::remove_file(&path) {
                    Ok(()) => { report.fix(format!("Removed tmp: {}", n)); count += 1; }
                    Err(e) => report.error(format!("Failed to remove {}: {}", n, e)),
                }
            }
        }
    }
    if count > 0 { report.ok(format!("Cleaned {} tmp files", count)); }
}
        ...
        for entry in entries.flatten() {
            ...
            if path.is_dir() {
                if let Ok(sub) = std::fs::read_dir(&path) {
                    for s in sub.flatten() { if s.path().is_file() { files.push(s.path()); } }
                }
            }
        }
    }
...
fn extract_hash_from_filename(name: &str) -> Option<String> {
    ...
    if stem.len() != 64 || !stem.chars().all(|c| c.is_ascii_hexdigit()) { return None; }
    Some(stem.to_string())
}
...
fn hex_to_bytes(hex: &str) -> Option<[u8; 32]> {
    ...
    let mut arr = [0u8; 32];
    for i in 0..32 { arr[i] = u8::from_str_radix(&hex[i*2..i*2+2], 16).ok()?; }
    Some(arr)
}
