...
fn test_registry_init_creates_secure_key_file() {
    ...
    let mut perms = fs::metadata(&key_path)
        .expect("Failed to read key file metadata")
        .permissions();
    ...
    assert_eq!(
        mode & 0o777,
        0o600,
        "Key file has wrong permissions: {:o}",
        mode
    );
...
fn test_registry_list_no_key_exposure() {
    use ed25519_dalek::SigningKey;
    use xudanu::crypto::server_identity::{ServerIdentity, TrustedServerRegistry};
    ...
    registry
        .add_server(server_id, &authority_key)
    ...
    let server = registry
        .get("test-server")
        .expect("Should have test server");
...
fn test_verify_server_identity_variable_naming() {
    use ed25519_dalek::SigningKey;
    use xudanu::crypto::server_identity::{
        verify_server_identity, ServerIdentity, TrustedServerRegistry,
    };
    ...
    assert_eq!(
        error_msg, "server identity verification failed",
...
fn test_key_file_permissions_security() {
    ...
    assert_eq!(
        mode & 0o077,
        0o000,
        "Key file should not be accessible by group/others"
    );
...
fn test_empty_registry_signature_verification() {
    ...
    assert!(
        verify_result.is_ok(),
        "Empty registry should have valid signature"
    );

    // Verify authority key matches
    assert_eq!(
        registry.authority_key,
...
fn test_timing_attack_resistance() {
    ...
    assert!(
        ratio < 10.0,
        "Timing difference too large: {}x (may indicate timing vulnerability)",
        ratio
    );

    // Both should return same generic error message
    let unknown_result =
        verify_server_identity("unknown-server", &unknown_key.to_bytes(), &registry);
    ...
    for (server_id, key) in test_cases {
        ...
        if let Err(msg) = result {
            ...
            assert!(
                ...
                "Error shouldn't mention signature"
            );
            assert!(
                !msg.contains("expired"),
                "Error shouldn't mention expiration"
            );
            assert!(
                !msg.contains("not found"),
                "Error shouldn't mention existence"
            );
            assert!(
                !msg.contains("invalid length"),
                "Error shouldn't mention length"
            );
            assert_eq!(
                msg, "server identity verification failed",
                "Error should be generic"
            );
        }
    }
...
fn test_registry_api_signing_key_required() {
    ...
    assert!(
        registry.verify_signature().is_ok(),
        "Registry should have valid signature"
    );
    ...
    assert!(
        updated.verify_signature().is_ok(),
        "Updated registry should have valid signature"
    );
}
...
fn test_input_validation_federation_domains() {
    ...
    for domain in valid_domains {
        ...
        assert_eq!(
            identity.federation_domain, domain,
...
fn test_key_material_protection() {
    ...
    file.save_to_file(&registry_path)
    ...
    let server_key_hex = hex::encode(server_key.to_bytes());
    assert!(
        file_content.contains(&server_key_hex),
        "Keys should be stored in file"
    );
    ...
    assert_eq!(
        mode & 0o044,
        0o000,
        "Registry file should not be readable by group/others"
    );
}
...
fn test_audit_trail_preservation() {
    ...
    use xudanu::crypto::server_identity::{
        ServerIdentity, ServerRegistryFile, TrustedServerRegistry,
    };
    ...
    // Load and verify
    let loaded =
        ServerRegistryFile::load_from_file(&registry_path).expect("Failed to load registry");
    assert_eq!(
        loaded.registry.last_updated, initial_timestamp,
        "Timestamp should be preserved"
    );
    assert!(
        loaded.registry.verify_signature().is_ok(),
    ...
    assert!(
        ...
        "Timestamp should increase after update"
    );
    ...
    updated_file
        ...
        .expect("Failed to save updated registry");

    // Load and verify audit trail is preserved
    let reloaded =
        ServerRegistryFile::load_from_file(&registry_path).expect("Failed to reload registry");
    assert_eq!(
        reloaded.registry.last_updated, updated_timestamp,
        "Updated timestamp should be preserved"
    );
    assert_eq!(
        reloaded.registry.server_count(),
        1,
        "Server count should be preserved"
    );
    assert!(
        reloaded.registry.verify_signature().is_ok(),
        "Updated signature should be valid"
    );
}
...
fn test_signature_integrity_multiple_operations() {
    ...
    // Remove a server
    registry = registry
        .remove_server("server-3", &authority_key)
        .expect("Failed to remove server");

    // Verify signature still valid
    assert!(
        registry.verify_signature().is_ok(),
        "Signature should be valid after removal"
    );

    // Verify final state
    assert_eq!(registry.server_count(), 4, "Should have 4 servers");
    assert!(
        !registry.is_trusted("server-3"),
        "Removed server should not be trusted"
    );
}
    ...
    match result {
        Ok(loaded) => {
            ...
            assert!(
                ...
                "Tampered registry should fail signature verification"
            );
        }
        Err(_) => {
...
fn test_concurrent_access_safety() {
    ...
    assert_eq!(
        final_registry.server_count(),
        10,
        "All servers should be added"
    );
    assert!(
        final_registry.verify_signature().is_ok(),
        "Final signature should be valid"
    );
}
