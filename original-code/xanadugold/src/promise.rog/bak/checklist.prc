class 1 Promise Root {
	1 Promise cast(IntValue catNum) login
	2 BooleanValue equals(Promise other) login
	3 IntValue hash() login
	4 BooleanValue isKindOf(IntValue catNum) login
...
message 416 TableStepper grants(IDRegion clubIDs, IDRegion globalIDs)
	417 BooleanValue isAcceptingConnections()
message 418 Void setGateLockSmith(LockSmith lockSmith)
message 419 Void shutDown()
}
...
message 253 Array copy() login
message 254 Array copy(IntValue count) login
...
message 257 Special export(IntValue count, IntValue start) login
	8 Promise get(IntValue index) login
...
function 265 FloatArray import(Special)
function 266 FloatArray zeros(IntValue precision, IntValue count)
message 267 IntValue precision()
}
class 6 HumberArray Array {
function 268 HumberArray import(Special) login
function 269 HumberArray zeros(IntValue count) login
}
class 7 IntArray Array {
function 10 IntArray import(Special) login
function 11 Array zeros(IntValue precision, IntValue count) login
message 12 IntValue precision()
}
class 8 PtrArray Array {
function 270 PtrArray import(Special)
...
class 10 ArrayBundle Bundle {
	14 Array array()
	15 OrderSpec ordering()
}
class 11 ElementBundle Bundle {
	16 RangeElement element()
}
...
message 46 Mapping combine(Mapping other)
	47 Region domain()
	294 CoordinateSpace domainSpace()
message 48 Mapping inverse()
	49 BooleanValue isComplete()
	50 BooleanValue isIdentity()
...
function 127 Work make(Edition contents)
	128 BooleanValue canRead()
	129 BooleanValue canRevise()
	130 ID editClub()
	131 Edition edition()
message 341 Void endorse(CrossRegion additionalEndorsements)
	132 CrossRegion endorsements()
...
message 359 Void setSignatureClub(ID club)
	360 ID signatureClub()
message 361 Edition sponsoredWorks()
	362 Edition sponsoredWorks(Filter filter)
}
class 49 RevisionDetector Promise {
event 3 NOACK revised(Work work, Edition contents, ID author, IntValue time, IntValue sequence)
}
...
event 4 NOACK grabbed(Work work, ID author, IntValue reason)
event 5 NOACK released(Work work, IntValue reason)
}
class 53 Stepper Promise {
	151 BooleanValue atEnd()
	152 Promise get()
...
class 59 HyperRef Wrapper {
	166 Work originalContext()
	167 Path pathContext()
...
function 177 SingleRef make(Edition material, Work workContext, Work originalContext, Path pathContext)
	178 Edition excerpt()
message 384 SingleRef withExceprt()
}
...
class 73 CrossRegion Region {
	217 Stepper boxes()
	218 BooleanValue isBox()
message 219 Region projection(IntValue index)
	220 PtrArray projections()
}
...
class 76 IntegerRegion Region {
	222 Stepper intervals()
message 398 Stepper intervals(OrderSpec order)
	223 BooleanValue isBoundedAbove()
	224 BooleanValue isBoundedBelow()
message 225 BooleanValue isInterval()
	226 IntValue start()
	227 IntValue stop()
}
class 77 RealRegion Region {
	228 Stepper intervals()
message 399 Stepper intervals(OrderSpec order)
	229 BooleanValue isBoundedAbove()
	230 BooleanValue isBoundedBelow()
...
class 78 SequenceRegion Region {
	234 Stepper intervals()
message 400 Stepper intervals(OrderSpec order)
	235 BooleanValue isBoundedAbove()
	236 BooleanValue isBoundedBelow()
message 237 BooleanValue isInterval()
	238 Sequence lowerEdge()
	239 IntValue lowerEdgePrefixLimit()
	240 IntValue lowerEdgeType()
	241 Sequence upperEdge()
	242 IntValue upperEdgePrefixLimit()
	243 IntValue upperEdgeType()
}
class 79 Value Promise {
}
class 80 FloatValue Value {
function 244 FloatValue import(Special)
message 401 IntValue precision()
}
class 81 IntValue Value {
function 245 IntValue import(Special) login
...
message 407 IntValue precision() login
