...
# sun4 case(s)
CXX = gcc -fno-defer-pop -munaligned-doubles


#CXXSYSINCLUDES =  -I/usr/local/lib/g++-include  -I/usr/local/lib/g++-include/sys -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include  -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include/sys 
#CXXSYSINCLUDES =  -I/usr/local/lib/g++-include  -I/usr/local/lib/g++-include/sys -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include  -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include/sys 
CXXSYSINCLUDES =  -I/usr/local/lib/g++-include  -I/usr/local/lib/g++-include/sys -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include  -I/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2/include/sys 
...
#COMPILER_SYS_DEFINES =  -DUSE_INLINE -DGNUSUN -DGNU  -Dsparc -Dunix   -Wall
COMPILER_SYS_DEFINES_foo =  -DUSE_INLINE -DGNUSUN  -DGNUSUN_TEST -DGNU  -Dsparc -Dunix 
#COMPILER_SYS_DEFINES =  $(COMPILER_SYS_DEFINES_foo)   -Wall
COMPILER_SYS_DEFINES =  $(COMPILER_SYS_DEFINES_foo)  -w
#COMPILER_SYS_DEFINES =   -DGNUSUN -DGNU  -Dsparc -Dunix  -Wall
#COMPILER_SYS_DEFINES =   -DGNUSUN -DGNU  -Dsparc -Dunix  -w
NM_FORMAT_ARG =

EXTRADEFS =
CXXEXTRAINCLUDES = $(CXXSYSINCLUDES)
CXXSRCINCLUDES = -I. -I.. -I$(XPPDIR)/xpp -I$(XPPDIR)/xlatexpp -I$(XPPDIR)/disk -I$(XPPDIR)/comm -I$(XPPDIR)/server -I$(XPPDIR)/disk -I$(XPPDIR)/urdi -I../sxx 
CXXINCLUDES =  $(CXXSRCINCLUDES) $(CXXEXTRAINCLUDES)
DEFINES = $(EXTRADEFS)
CXXDEFINES_foo = $(DEFINES) $(COMPILER_SYS_DEFINES_foo)
CXXDEFINES = $(DEFINES) $(COMPILER_SYS_DEFINES)
...
#       C language section
#
CC        = gcc -fno-defer-pop  -munaligned-doubles
...
#CXXPLATFORMLIBS=  -L/usr/local/lib -L/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2 -lgcc -liostream    -liberty 

#CXXPLATFORMLIBS=  -L/usr/local/lib -L/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2  -lgcc -lg++ -liostream    -liberty
CXXPLATFORMLIBS=   -L/usr/local/lib -lgcc -liostream -lstdc++ -lg++ -lm
#CXXPLATFORMLIBS=   -L/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2 -L/usr/local/lib   -liostream -lstdc++ -lg++ -lgcc  -lm
#CXXPLATFORMLIBS=   -L/usr/local/lib/gcc-lib/sparc-sun-sunos4.1.4/2.7.2 -L/usr/local/lib  -lgcc -liostream -lm -lc -lC
...
#	$(STUBBLE) $(EXTRASTUBBLEDEFS) $(INCLUDES) $(CXXDEFINES_foo) $< -o../sxx/$@

../sxx/%.sxx : ../%.hxx $(STUBBLEDIR)/stubblef.f $(EXTRASTUBBLEDEPENDS)
	$(STUBBLE) $(EXTRASTUBBLEDEFS) $(CXXINCLUDES) $(CXXDEFINES_foo) $< -o$@

%.ppo : ../%.hxx $(STUBBLEDIR)/stubblef.f $(EXTRASTUBBLEDEPENDS)
	$(STUBBLE) -p $(EXTRASTUBBLEDEFS) $(CXXINCLUDES) $(CXXDEFINES_foo) $<


%.sxx : %.hxx $(STUBBLEDIR)/stubblef.f $(EXTRASTUBBLEDEPENDS)
	asynch ; $(STUBBLE) $(EXTRASTUBBLEDEFS) $(CXXINCLUDES) $(CXXDEFINES_foo) $< -o../sxx/$@

%.ppo : %.hxx $(STUBBLEDIR)/stubblef.f $(EXTRASTUBBLEDEPENDS)
	asynch ; $(STUBBLE) -p $(EXTRASTUBBLEDEFS) $(CXXINCLUDES) $(CXXDEFINES_foo) $<
...
depends :
	-xmakedep -o - -R.hxx.sxx $(CXXDEFINES) $(CXXSRCINCLUDES) \
		'-DCOPY:="copy.hf"' '-DPSEUDO_COPY:="copy.hf"' \
		-I$(XPPDIR)/stubble $(SYSINCLUDES) -W `ls *.cxx ../*.cxx` \
		| sed -e 's|^.*/\([a-z0-9]*\)\.sxx|        ../sxx/\1.sxx|' > depends.mk
	-xmakedep -o - $(CXXSRCINCLUDES)  $(SYSZZZINCLUDES) \
		-W `ls *.c ../*.c` \
		| sed -e 's|^.*/\([a-z0-9]*\)\.sxx|        ../sxx/\1.sxx|' >> depends.mk
	-xmakedep -o - -e sxx $(CXXDEFINES_foo) -DSTUBBLE $(CXXSRCINCLUDES) \
