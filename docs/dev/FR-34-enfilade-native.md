...
| Recorders: crum-guided incremental backfollow | **Active** (2026-08-31, Phase A): `OrglRoot::crum_diff_visit` (parallel subtree descent prunes equal crums; common prefix/suffix content trim inside differing leaves — single-leaf from_text editions get proportional visits without subtree structure). `update_work`/`update_work_with_parent` now apply COUNT-BASED deltas to both indexes (was O(N) full walk + per-entry retains — quadratic churn per edit; measured 347µs incremental vs 64.2ms full for one edit on 15k chars, 185×; the full path extrapolates to ~26s at 240k). Armor: exact equivalence with full re-registration (content-key + fingerprint probes), pruning proof, release-gated cost guard | Gold's recorders, edit-side |
| Recorders: persistence journal | **Active — engine core** (2026-08-31, Phase B): `BackfollowSnapshot` (plain-data index state: content-key map, fingerprint map, link registrations — postcard round-trip armored) + `WorkIndexDelta` (the journalable unit, already computed by `update_work_delta`) + `RecorderJournalEntry` replay (content-free deltas; WorkAdded replays from the restored work). `register_work_meta_only` rebuilds crums/dagwood metadata in O(works) — the Arc-linked canopy structures are deliberately NOT journaled. Armor: snapshot + journal replay == full rebuild (queries + fingerprint probes, 4-edit history); serialization round trip. Server wiring (journal file lifecycle at checkpoint/restore, WAL-convention) is the adoption step | RecorderFossil proper |
 **Active** (2026-08-31): `space/lattice_wire.rs` — postcard-encoded units + tombstones (addresses as number vectors; regions as edge descriptors round-tripped via interval/above constructors). Byte-exact round-trip armor; MEASURED proportionality (small edit on large fragmented doc < 10% of full-state bytes); E2E: two replicas reconcile exclusively over wire bytes (encode → decode_and_apply), crums and text converge. Transport adoption = FederationFrame plumbing at cutover | Wire Recipe/Cookbook lineage |
...
| Crum-path notarization | **Active** (2026-08-31): `edition/notarize.rs` — `RangeNotarization` binds (range crum, root crum, excerpt hash, bounds) under a server Ed25519 signature. Prove WHAT was quoted without the document (excerpt hash — the privacy property); prove THAT it was there (range crum recomputes against the edition); historical attestations survive drift (signature + excerpt remain valid after edits). Armor: lifecycle (quote→notarize→verify→drift-invalidate), empty/inverted ranges, foreign-key rejection | Gold never had this — modern hash commitments on the crum idea |
