...
| Recorders: persistence journal | **Designed** (Phase B): restore currently rebuilds by full re-registration; the journal records (register/update/unregister) events appended per mutation, compacted at checkpoint, replayed on restore. Startup cost O(journal) not O(all works × entries) | RecorderFossil proper |
